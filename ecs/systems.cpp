...
namespace mrg {
void Renderer::render(const std::unique_ptr<sf::RenderWindow> &window, std::unique_ptr<sf::View> &cameraView,
...
// important: if registry will be const, the view will be const and the modifications of the components' data won't
// work
void Movement::update(entt::registry &registry, const real dt) {
    ...
    view.each([&](auto &rigidBody, auto &moveable) {
        glm::vec3 force = moveable.direction * (100000000000 * dt);
...
bool Physics::resolveCollisions(RigidBody &rbFirst, RigidBody &rbSecond, Transform &tFirst, Transform &tSecond,
                                const real dt) {
    ...
    j /= rbFirst.inverseMass + rbSecond.inverseMass; // if axis wasn't normalized, magnitude was needed in the
    // denominator, multiplied by the inverse masses sum
    const glm::vec3 impulse = axisNormalized * j;
    if (!rbFirst.hasInfiniteMass())
        rbFirst.velocity += impulse * (-rbFirst.inverseMass * Constants::FORCE_SCALAR * dt);
    if (!rbSecond.hasInfiniteMass())
        rbSecond.velocity += impulse * (rbSecond.inverseMass * Constants::FORCE_SCALAR * dt); // incremented position before
    return true;
}
...
void Physics::update(entt::registry &registry, const real dt) {
    ...
    for (entt::entity first : view) {
        ...
        // but will calculate collisions for non infinite mass objects with infinite mass one
        if (rbFirst.hasInfiniteMass()) continue;

        // getting entity's position on the grid
        // TODO: get coordinates relative to the window and not on the absolute position - relevant when maps are larger than the window size
        int firstGridX = static_cast<int>(tFirst.position.x);
        int firstGridY = static_cast<int>(tFirst.position.y);

        // traversing other objects that are different and are close by in the window grid
        for (entt::entity second : view) {
            ...
            // getting entity's position on the grid and checking if we should calculate the collision with another
            int secondGridX = static_cast<int>(tSecond.position.x);
            int secondGridY = static_cast<int>(tSecond.position.y);

            int distance = manhattanDistance(firstGridX, secondGridX, firstGridY, secondGridY);
            if (distance > gridCheckMaxDistance) continue;
    ...
    view.each([&](auto &rigidBody, auto &transform) {
        ...
        transform.position += rigidBody.velocity * dt + rigidBody.acceleration * dt * dt * static_cast<real>(0.5);
        // defined friction
        rigidBody.velocity *= static_cast<real>(1) / Constants::GRASS_FRICTION_DEGRADATION_CONSTANT;
        // clamping velocity
        const real velocityMagnitude = glm::dot(rigidBody.velocity, rigidBody.velocity);
